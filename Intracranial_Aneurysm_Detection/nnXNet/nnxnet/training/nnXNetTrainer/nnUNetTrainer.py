...
from nnxnet.training.data_augmentation.custom_transforms.cascade_transforms import MoveSegAsOneHotToData, \
    ApplyRandomBinaryOperatorTransform, RemoveRandomConnectedComponentFromOneHotEncodingTransform
from nnxnet.training.data_augmentation.custom_transforms.deep_supervision_donwsampling import \
    DownsampleSegForDSTransform2
from nnxnet.training.data_augmentation.custom_transforms.limited_length_multithreaded_augmenter import \
    LimitedLenWrapper
from nnxnet.training.data_augmentation.custom_transforms.masking import MaskTransform
from nnxnet.training.data_augmentation.custom_transforms.region_based_training import \
    ConvertSegmentationToRegionsTransform
from nnxnet.training.data_augmentation.custom_transforms.transforms_for_dummy_2d import Convert2DTo3DTransform, \
    Convert3DTo2DTransform
from nnxnet.training.dataloading.data_loader_2d import nnXNetDataLoader2D
from nnxnet.training.dataloading.data_loader_3d import nnXNetDataLoader3D
from nnxnet.training.dataloading.nnxnet_dataset import nnXNetDataset
from nnxnet.training.dataloading.utils import get_case_identifiers, unpack_dataset
...
from nnxnet.utilities.crossval_split import generate_crossval_split
from nnxnet.utilities.default_n_proc_DA import get_allowed_n_proc_DA
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.preprocessed_dataset_folder_base = join(nnXNet_preprocessed, self.plans_manager.dataset_name) \
            if nnXNet_preprocessed is not None else None
        self.output_folder_base = join(nnXNet_results, self.plans_manager.dataset_name,
                                       self.__class__.__name__ + '__' + self.plans_manager.plans_name + "__" + configuration) \
            if nnXNet_results is not None else None
        self.output_folder = join(self.output_folder_base, f'fold_{fold}')

        self.preprocessed_dataset_folder = join(self.preprocessed_dataset_folder_base,
                                                self.configuration_manager.data_identifier)
        # unlike the previous nnxnet folder_with_segs_from_previous_stage is now part of the plans. For now it has to
        # be a different configuration in the same plans
        # IMPORTANT! the mapping must be bijective, so lowres must point to fullres and vice versa (using
        # "previous_stage" and "next_stage"). Otherwise it won't work!
        self.is_cascaded = self.configuration_manager.previous_stage_name is not None
        self.folder_with_segs_from_previous_stage = \
            join(nnXNet_results, self.plans_manager.dataset_name,
        ...
        self.print_to_log_file("\n#######################################################################\n"
                               "Please cite the following two papers when using nnX-Net:\n"
                               "Isensee, F., Jaeger, P. F., Kohl, S. A., Petersen, J., & Maier-Hein, K. H. (2021). "
                               "nnU-Net: a self-configuring method for deep learning-based biomedical image segmentation. "
                               "Nature methods, 18(2), 203-211.\n"
                               "nnX-Net: an extensible multi-task learning framework for medical imaging. "
    ...
    def _do_i_compile(self):
        ...
        if 'nnXNet_compile' not in os.environ.keys():
            return True
        else:
            return os.environ['nnXNet_compile'].lower() in ('true', '1', 't')
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        variants can be loaded at inference time (inference will use the same nnXNetTrainer that was used for
    ...
    def plot_network_architecture(self):
        if self._do_i_compile():
            self.print_to_log_file("Unable to plot network architecture: nnXNet_compile is enabled!")
            return
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            dataset = nnXNetDataset(self.preprocessed_dataset_folder, case_identifiers=None,
    ...
    def get_tr_and_val_datasets(self):
        ...
        dataset_tr = nnXNetDataset(self.preprocessed_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                   num_images_properties_loading_threshold=0)
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            dl_tr = nnXNetDataLoader2D(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None)
            dl_val = nnXNetDataLoader2D(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
            ...
            dl_tr = nnXNetDataLoader3D(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None)
            dl_val = nnXNetDataLoader3D(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
