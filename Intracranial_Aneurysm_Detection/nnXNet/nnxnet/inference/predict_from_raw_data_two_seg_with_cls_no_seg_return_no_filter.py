...
import nnxnet
from nnxnet.configuration import default_num_processes
from nnxnet.inference.data_iterators import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
    preprocessing_iterator_fromnpy
from nnxnet.inference.export_prediction import export_prediction_from_logits, \
    convert_predicted_logits_to_segmentation_with_correct_shape
from nnxnet.inference.sliding_window_prediction import compute_gaussian, \
    compute_steps_for_sliding_window
from nnxnet.utilities.file_path_utilities import get_output_folder, check_workers_alive_and_busy
...
from nnxnet.utilities.utils import create_lists_from_splitted_dataset_folder
...
class nnXNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        self.seg_index_1 = configuration_manager.seg_index_1
        self.seg_index_2 = configuration_manager.seg_index_2
    ...
    def manual_initialization(self, network: nn.Module, plans_manager: PlansManager,
        ...
        """
        This is used by the nnXNetTrainer to initialize nnXNetPredictor for the final validation
        """
        self.plans_manager = plans_manager
        self.configuration_manager = configuration_manager
        self.list_of_parameters = parameters
        ...
        self.label_manager = plans_manager.get_label_manager(dataset_json)
        allow_compile = True
        allow_compile = allow_compile and ('nnXNet_compile' in os.environ.keys()) and (
                    os.environ['nnXNet_compile'].lower() in ('true', '1', 't'))
    ...
    def initialize_network_and_gaussian(self):
        ...
        if isinstance(self.network, OptimizedModule):
            ...
            self.network.load_state_dict(self.list_of_parameters[0], strict=False )
    ...
    def predict_from_files(self,
                           ...
                           part_id: int = 0):
        """
        This is nnX-Net's default function for making predictions. It works best for batch predictions
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        input_image: Make sure to load the image in the way nnX-Net expects! nnX-Net is trained on a certain axis
                     ...
                     for loading images as was used during nnX-Net preprocessing! You can find that class in your
        ...
        if self.verbose:
            print('predicting')
        cls_predicted_logits_list = self.predict_logits_from_preprocessed_data(dct['data'])
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        if cls_pred_list is None:
            cls_pred_list = self.predict_sliding_window_return_logits(data)
            for t_i in range(self.num_cls_task):
                cls_pred_list[t_i] = cls_pred_list[t_i].to('cpu')
        else:
            cls_pred_list_tmp = self.predict_sliding_window_return_logits(data)
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            raise e
        return cls_predicted_logits_list
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.perform_everything_on_device and self.device != 'cpu':
                ...
                try:
                    cls_predicted_logits_list = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                           self.perform_everything_on_device)
                except RuntimeError:
                    print(
                        'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                    empty_cache(self.device)
                    cls_predicted_logits_list = self._internal_predict_sliding_window_return_logits(data, slicers, False)
            else:
                cls_predicted_logits_list = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                       self.perform_everything_on_device)
...
def predict_entry_point_modelfolder():
    import argparse
    parser = argparse.ArgumentParser(description='Use this to run inference with nnX-Net. This function is used when '
                                                 'you want to manually specify a folder containing a trained nnX-Net '
                                                 'model. This is useful when the nnxnet environment variables '
                                                 '(nnXNet_results) are not set.')
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                        help="Use this to set the device the inference should run with. Available options are 'cuda' "
                             ...
                             "Use CUDA_VISIBLE_DEVICES=X nnXNet_predict [...] instead!")
    ...
    print(
        ...
        "nnX-Net: a self-configuring method for deep learning-based biomedical image segmentation. "
    ...
    if args.device == 'cpu':
        ...
        # multithreading in torch doesn't help nnX-Net if run on GPU
    ...
    predictor = nnXNetPredictor(tile_step_size=args.step_size,
                                use_gaussian=True,
...
def predict_entry_point():
    ...
    parser.add_argument('-p', type=str, required=False, default='nnXNetPlans',
                        help='Plans identifier. Specify the plans in which the desired configuration is located. '
                             'Default: nnXNetPlans')
    parser.add_argument('-tr', type=str, required=False, default='nnXNetTrainer',
                        help='What nnX-Net trainer class was used for training? Default: nnXNetTrainer')
    parser.add_argument('-c', type=str, required=True,
                        help='nnX-Net configuration that should be used for prediction. Config must be located '
    ...
    parser.add_argument('-num_parts', type=int, required=False, default=1,
                        help='Number of separate nnXNet_predict call that you will be making. Default: 1 (= this one '
                             'call predicts everything)')
    parser.add_argument('-part_id', type=int, required=False, default=0,
                        help='If multiple nnXNet_predict exist, which one is this? IDs start with 0 can end with '
                             'num_parts - 1. So when you submit 5 nnXNet_predict calls you need to set -num_parts '
    ...
    assert args.part_id < args.num_parts, 'Do you even read the documentation? See nnXNet_predict -h.'
...
if __name__ == '__main__':
    ...
    from nnxnet.paths import nnXNet_results, nnXNet_raw
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnXNet_results, 'Dataset003_Liver/nnXNetTrainer__nnXNetPlans__3d_lowres'),
        use_folds=(0,),
        checkpoint_name='checkpoint_final.pth',
    )
    predictor.predict_from_files(join(nnXNet_raw, 'Dataset003_Liver/imagesTs'),
                                 join(nnXNet_raw, 'Dataset003_Liver/imagesTs_predlowres'),
    ...
    img, props = SimpleITKIO().read_images([join(nnXNet_raw, 'Dataset003_Liver/imagesTr/liver_63_0000.nii.gz')])
    ...
    #     )
    # predictor.initialize_from_trained_model_folder(
    #     join(nnXNet_results, 'Dataset003_Liver/nnXNetTrainer__nnXNetPlans__3d_cascade_fullres'),
    #     use_folds=(0,),
    #     checkpoint_name='checkpoint_final.pth',
    # )
    # predictor.predict_from_files(join(nnXNet_raw, 'Dataset003_Liver/imagesTs'),
    #                              join(nnXNet_raw, 'Dataset003_Liver/imagesTs_predCascade'),
    #                              save_probabilities=False, overwrite=False,
    #                              num_processes_preprocessing=2, num_processes_segmentation_export=2,
    #                              folder_with_segs_from_prev_stage='/media/isensee/data/nnXNet_raw/Dataset003_Liver/imagesTs_predlowres',
    #                              num_parts=1, part_id=0)
