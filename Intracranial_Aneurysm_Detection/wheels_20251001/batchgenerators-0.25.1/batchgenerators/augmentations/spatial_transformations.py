...
from batchgenerators.augmentations.crop_and_pad_augmentations import random_crop_with_gpe as random_crop_aug_with_gpe
from batchgenerators.augmentations.crop_and_pad_augmentations import center_crop_with_gpe as center_crop_aug_with_gpe
...
def augment_spatial_with_gpe(data, seg, gpe, patch_size, patch_center_dist_from_border=30,
                    do_elastic_deform=True, alpha=(0., 1000.), sigma=(10., 13.),
                    do_rotation=True, angle_x=(0, 2 * np.pi), angle_y=(0, 2 * np.pi), angle_z=(0, 2 * np.pi),
                    do_scale=True, scale=(0.75, 1.25), border_mode_data='nearest', border_cval_data=0, order_data=3,
                    border_mode_seg='constant', border_cval_seg=0, order_seg=0, border_mode_gpe='constant', 
                    border_cval_gpe=-2, order_gpe=3, random_crop=True, p_el_per_sample=1,
                    p_scale_per_sample=1, p_rot_per_sample=1, independent_scale_for_each_axis=False,
                    p_rot_per_axis: float = 1, p_independent_scale_per_axis: int = 1):
    dim = len(patch_size)
    ...
    if dim == 2:
        ...
        if seg is not None:
            seg_result = np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
        if gpe is not None:
            gpe_result = np.zeros((gpe.shape[0], gpe.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
    else:
        data_result = np.zeros((data.shape[0], data.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                               dtype=np.float32)
        if seg is not None:
            seg_result = np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
        if gpe is not None:
            gpe_result = np.zeros((gpe.shape[0], gpe.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            # 对gpe进行插值（如果存在）- 新增部分
            if gpe is not None:
                for channel_id in range(gpe.shape[1]):
                    gpe_result[sample_id, channel_id] = interpolate_img(gpe[sample_id, channel_id], coords, order_gpe,
                                                                        border_mode_gpe, cval=border_cval_gpe)
        else:
            # 如果没有应用坐标变换，则进行随机裁剪或中心裁剪
            if seg is None:
                s = None
            else:
                s = seg[sample_id:sample_id + 1]
            ...
            if gpe is None:
                ...
                g = gpe[sample_id:sample_id + 1]
            
            if random_crop:
                margin = [patch_center_dist_from_border[d] - patch_size[d] // 2 for d in range(dim)]
                d, s, g = random_crop_aug_with_gpe(data[sample_id:sample_id + 1], s, g, patch_size, margin)
            else:
                d, s, g = center_crop_aug_with_gpe(data[sample_id:sample_id + 1], patch_size, s, g)
            
            data_result[sample_id] = d[0]
            if seg is not None:
                seg_result[sample_id] = s[0]
            if gpe is not None:
                gpe_result[sample_id] = g[0]
    
    # 返回结果
    if seg is not None and gpe is not None:
        return data_result, seg_result, gpe_result
    elif seg is not None:
        return data_result, seg_result, None
    elif gpe is not None:
        return data_result, None, gpe_result
    else:
        return data_result, None, None
