...
def center_crop_with_gpe(data, crop_size, seg=None, gpe=None):
    return crop_with_gpe(data, seg, gpe, crop_size, 0, "center")
...
def crop_with_gpe(data, seg=None, gpe=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
                 pad_mode='constant', pad_kwargs={'constant_values': 0},
                 pad_mode_seg='constant', pad_kwargs_seg={'constant_values': 0},
                 pad_mode_gpe='constant', pad_kwargs_gpe={'constant_values': -2000}):
    """
    同时裁剪data, seg和gpe的增强版本
    """
    if not isinstance(data, (list, tuple, np.ndarray)):
        raise TypeError("data has to be either a numpy array or a list")
    ...
    if seg is not None:
        ...
        assert all([i == j for i, j in zip(seg_shape[2:], data_shape[2:])]), "data and seg must have the same spatial dimensions"

    # 检查gpe
    if gpe is not None:
        gpe_shape = tuple([len(gpe)] + list(gpe[0].shape))
        gpe_dtype = gpe[0].dtype
        if not isinstance(gpe, (list, tuple, np.ndarray)):
            raise TypeError("gpe has to be either a numpy array or a list")
        assert all([i == j for i, j in zip(gpe_shape[2:], data_shape[2:])]), "data and gpe must have the same spatial dimensions"

    if type(crop_size) not in (tuple, list, np.ndarray):
        crop_size = [crop_size] * dim
    else:
        assert len(crop_size) == dim, "crop_size must have the same dimension as your data"
    ...
    if gpe is not None:
        gpe_return = np.zeros([gpe_shape[0], gpe_shape[1]] + list(crop_size), dtype=gpe_dtype)
    else:
        gpe_return = None
    ...
    for b in range(data_shape[0]):
        ...
        if seg is not None:
            seg_shape_here = [seg_shape[0]] + list(seg[b].shape)
        if gpe is not None:
            gpe_shape_here = [gpe_shape[0]] + list(gpe[b].shape)
        ...
        # 裁剪gpe
        if gpe is not None:
            slicer_gpe = [slice(0, gpe_shape_here[1])] + [slice(lbs[d], ubs[d]) for d in range(dim)]
            gpe_cropped = gpe[b][tuple(slicer_gpe)]
        ...
        if any([i > 0 for j in need_to_pad for i in j]):
            ...
            if seg is not None:
                seg_return[b] = np.pad(seg_cropped, need_to_pad, pad_mode_seg, **pad_kwargs_seg)
            if gpe is not None:
                gpe_return[b] = np.pad(gpe_cropped, need_to_pad, pad_mode_gpe, **pad_kwargs_gpe)
        else:
            data_return[b] = data_cropped
            if seg is not None:
                seg_return[b] = seg_cropped
            if gpe is not None:
                gpe_return[b] = gpe_cropped

    return data_return, seg_return, gpe_return

def random_crop(data, seg=None, crop_size=128, margins=[0, 0, 0]):
    return crop(data, seg, crop_size, margins, 'random')

def random_crop_with_gpe(data, seg=None, gpe=None, patch_size=None, margin=None):
    return crop_with_gpe(data, seg, gpe, crop_size=patch_size, margins=margin, crop_type="random")
